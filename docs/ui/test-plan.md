...
| UI-BOARD-001 | 主棋盘和地图 | 地图加载、道路可见、棋盘缩放后坐标映射正确、hover 可建造空格时显示当前选中塔预览、敌人/塔/血条/攻击反馈可见；Single/Area/Slow/Flame 圆形塔顶资源居中，运行时按当前目标旋转；每种塔的投射物和命中特效可见。 | GUT 覆盖地图资源、默认路径、圆形塔顶资源尺寸、坐标映射、放置、hover 放置预览显示条件、敌人生成、攻击反馈和 Single/Flame 资产读取；native UI smoke 覆盖放置预览、一次放塔和敌人生成；native gameplay smoke 从塔配置自动发现逐塔视觉目录，覆盖 Single 放塔、攻击投射物、Flame 灼烧和击杀奖励截图。 | `*-tower-placement-preview.png` 和 overlay；UI smoke 整屏截图；gameplay smoke `tower_visual_catalog` 的 `*-tower-ready`、`*-projectile-visible`、`*-impact-effect` focus crop/overlay，以及其他 gameplay scenario board/focus crop。 | 已放置塔、敌人、投射物和命中特效视觉审查依赖 gameplay smoke 的 board/focus crop；尚无像素级 golden baseline。 |
...
| UI-TOWER-001 | 塔选择卡组 | Single/Area/Slow/Flame 四张卡；`1`/`2`/`3`/`4` 数字键切塔；选中态；暂停禁用；金币不足禁用；图标、描述、费用和 tooltip；侧栏/底栏响应式布局。 | GUT 覆盖按钮选塔、数字键选塔、放置塔类型、金币不足禁用、暂停禁用和响应式布局；native smoke 从 `TowerConfig.get_visual_test_tower_specs()` 自动遍历塔卡选中态，并覆盖金币不足禁用视觉状态。 | `*-tower-deck.png`、`*-tower-deck-*-selected.png`、`*-tower-deck-insufficient-gold.png` 和对应 overlay；`report.md` Tower deck checklist。 | 暂停禁用的塔卡视觉由 pause overlay 状态间接覆盖；塔卡 UI 当前仍是四塔固定按钮，新增第五种塔时需要先扩展 HUD。 |
...
| UI-RESP-001 | 响应式布局 | 桌面、移动横屏、方形/紧凑视口下 HUD、棋盘、塔卡、status/hint 互不遮挡且可读可点；四塔卡在窄底栏自动 2x2。 | GUT 覆盖移动横屏、方形底部塔卡、紧凑消息区和坐标映射；native smoke 覆盖三个固定视口。 | UI smoke 三个视口整屏截图和 15 组 crop/overlay；gameplay smoke 每个 checkpoint 额外产出 board/focus crop 和辅助线 overlay。 | 未覆盖竖屏手机；如果支持竖屏，需要新增视口和布局契约。 |
