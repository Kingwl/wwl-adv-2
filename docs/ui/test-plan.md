...
| --- | --- | --- | --- | --- | --- |
| UI-START-001 | 开始界面和选关 | 标题、crest、装饰框、`Start` 按钮；点击 `Start` 展开 5 个 MVP 关卡按钮；点击关卡进入主场景；`Back` 返回开始态；桌面和紧凑视口居中可读。 | GUT 检查入口场景、标题、按钮文本、5 个关卡按钮、Back 和选中关卡 meta；native smoke 通过开始按钮进入选关态、截图局部选关面板，再点击 Level 1 进入主场景。 | `*-start-screen.png`、`*-start-screen-full.png`、`*-level-select.png` 和对应 overlay；4x4 visual-review key crops。 | native smoke 只点击 Level 1 进入主场景；Level 2-5 可进入性由 GUT 关卡目录/selected-level 断言和 level grid audit 地图截图补充覆盖。 |
...
| UI-MENU-001 | 暂停菜单 | `Menu` 或 `ui_cancel`/`Esc` 打开暂停 overlay；塔操作菜单打开时 `Esc` 先关闭菜单；`Resume` 继续；`Select` 返回选关；暂停时塔卡和菜单状态同步。 | GUT 覆盖暂停、恢复、按钮文案、返回选关、状态同步和 Esc 优先级；native smoke 进入暂停状态截图。 | `*-pause-overlay.png` 和 `*-pause-overlay-overlay.png`。 | 暂停状态下塔卡禁用视觉没有单独 tower deck crop；如果禁用样式变化再补。 |
| UI-WIN-001 | 胜利 overlay | 全部波次清空后显示 `Victory`；有下一关时 `Next` 进入下一关、`Restart` 重开本关；最终关 `Select` 返回选关；status 显示胜利。 | GUT 覆盖胜利状态、下一关路径、默认胜利按钮、最终关无下一关按钮和 overlay 文案；native smoke 进入胜利 overlay 截图。 | `*-victory-overlay.png` 和 `*-victory-overlay-overlay.png`。 | native smoke 直接调用场景方法进入视觉状态，不模拟完整打通最终波次；`Next` 按钮换关由路径/meta 断言覆盖，不在 smoke 中实际切关。 |
| UI-LOSE-001 | 失败 overlay | 生命归零后显示 `Defeat`；`Restart` 重开；`Select` 返回选关；status 显示失败。 | GUT 覆盖失败状态、overlay 文案和按钮；native smoke 进入失败 overlay 截图。 | `*-defeat-overlay.png` 和 `*-defeat-overlay-overlay.png`。 | native smoke 直接调用场景方法进入视觉状态，不模拟完整漏怪到 0 生命。 |
