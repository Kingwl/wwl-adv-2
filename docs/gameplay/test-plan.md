...
| GP-WAVE-001 | 波次和胜利 | 波次从 JSON 加载并引用有效 enemy type；按间隔生成；大 delta 不跳过；波次清空；进入下一波；全部波次清空胜利。 | `test_wave_config.gd`、`test_wave_spawner.gd`、`test_combat_simulation.gd`、`test_main_scene.gd`、`check-gameplay-smoke.sh` | GUT 断言 wave config、spawn result、wave clear event、victory state；asset check 校验 wave schema 和 enemy reference；gameplay smoke 覆盖清波胜利 checkpoint。 | MVP 长波次内容和平衡快照。 |
| GP-DATA-001 | 关卡、地图和玩法数据 | 默认关卡路径、地图 style、economy JSON、enemies JSON、waves JSON 和 towers JSON 可加载，非法数据被 schema 或语义检查拦截。 | `test_map_definitions.gd`、`test_economy_config.gd`、`test_enemy_config.gd`、`test_wave_config.gd`、`test_tower_config.gd`、`check-assets.sh` | GUT 和 `check-all.sh` 的资产检查；校验关卡/style、经济、敌人、波次、塔、资源路径和跨文件引用。 | 还缺长局 replay snapshot 和更细的平衡数据门禁。 |
