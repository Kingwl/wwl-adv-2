...
- Godot: 4.6.2 stable.
- GUT: 9.6.0.
- GUT 套件：221 个测试通过，1642 个断言。
- Asset check：5 个关卡、5 个 map style、3 类敌人、6 个 wave set、48 个波次、5 座塔和 1 份经济配置通过。
- Native UI smoke：桌面、移动横屏和方形视口通过，每个视口 19 个 review crop；截图、局部 crop、overlay 和 4x4 visual-review contact sheet 位于 `ci-artifacts/ui-smoke/native/`。
...
- Structural lint：Tree-sitter GDScript 解析 106 个文件，0 个 error，0 个 warning。
...
- 波次生成和波次清空事件。
- 玩家生命、胜利和失败状态。
- 开始场景、5 个 MVP 关卡选关面板、主场景、暂停菜单、重开、返回选关、胜利后进入下一关、最终关返回选关和失败流程。
...
- 地图 deterministic grid layer 合成工具：`game/tools/compose-map-grid-layer.py` 可从关卡 JSON 和现有 image-generated 背景纹理生成 clean playfield 背景、透明 `grid_layer_composed.png`、flattened preview 和 manifest；Level 2-5 已接入运行时 map style，其中 Level 2-4 使用外圈 image-generated 场景加中场确定性石板空地，Level 5 使用 clean citadel courtyard 背景；边缘路径端点会生成 stone apron / threshold，style 可用 `grid_layer_hidden_blocker_cells` 隐藏水渠/城墙等场景边界上的视觉 blocker。
...
- Map style 支持可选 `grid_layer` 全棋盘覆盖图、`grid_layer_hidden_blocker_cells` 视觉 blocker 排除列表，以及 `tiles.buildable/path/blocked/locked` 逐 slot overlay；`BoardMapRenderer` 先画 background，再画 `grid_layer`，最后画逐 slot tile。
...
- 覆盖 start-to-level-select-to-main 可玩性、响应式视口、单塔放置、塔操作菜单和截图产物的 native UI smoke。
...
- 当前敌人和波次已数据化，但只有三类基础敌人；MVP 五关已有第一版差异化地图、路径、blocked mask 和玩家可选关卡入口，仍缺实现后的长局平衡快照。Level 2-5 已切到 clean background + deterministic grid layer；后续长局平衡需要确认每关可建造密度是否合适。
...
2. 在雷霆塔前收尾剩余 `GameTower.Type` 兼容边界：优先处理场景/渲染 fallback、颜色和旧合成测试中的 tower-type match。
3. 为 Level 2-5 增加 per-level gameplay smoke 或 headless balance summary，让选关后的长局体验有自动化快照。
