...
| TD-011 | 塔内容扩展性 | 塔 roster 虽已放入 `towers.json`，但部分兼容路径仍保留 `GameTower.Type` enum 和若干 tower-type match。 | Medium | TBD | Mitigated | `TowerConfig` 以 tower id 作为 roster 主键；schema 和 `check-assets.py` 不再枚举固定五塔 type；HUD 按钮、数字键、smoke 选塔、核心放置、核心攻击、投射物事件和资产 catalog 已可按 tower id 读取非 enum 塔定义。剩余边界主要是现有五塔的场景/渲染 fallback、颜色和旧合成测试。 |
...
| TD-016 | 校验规则重复 | enemy/wave enum、tower effect 规则和 tier 语义同时存在于 JSON schema、`check-assets.py` 和 GDScript validator，存在规则漂移风险。 | Medium | TBD | Open | tower roster type 已改为 id 数据契约，不再由 schema/Python 枚举固定五塔；仍需明确 schema/Python/GDScript 对 effect、status、damage、enemy 和 wave 语义校验的职责边界。 |
