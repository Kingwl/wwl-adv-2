...
| TD-004 | 内容数据 | 硬编码敌人/波次数值会让平衡和回归测试变贵。 | Medium | TBD | Mitigated | 敌人、波次和经济第一版已迁移到 `game/data/`，并纳入 schema/GUT/asset check；后续主要债务转为长波次和平衡 fixture。 |
...
| TD-009 | 配置来源 | 若 session 重新内联默认经济或波次配置，会绕过数据门禁并掩盖配置错误。 | Medium | TBD | Mitigated | `BoardGameSession` 现在从 `EconomyConfig`、`EnemyCatalog` 和 `WaveConfig` 加载 `game/data/economy`、`game/data/enemies` 和 `game/data/waves`，并不再回退到硬编码三波。 |
...
| TD-011 | 塔内容扩展性 | 塔 roster 虽已放入 `towers.json`，但新增塔仍需要改 `GameTower.Type` enum、tower schema、`check-assets.py` 枚举和若干 tower-type match。 | Medium | TBD | Open | 建造费用和视觉资源已进入塔数据并由 `BoardAssetCatalog` 读取；雷霆塔前继续评估是否用 tower id 驱动运行时和 schema。 |
| TD-012 | 塔配置实现 | `TowerConfig` 曾同时承担 JSON 读取、解析、语义校验、默认值、UI button spec 和旧 tier/effect 兼容路径，文件已成为核心配置修改的高风险点。 | Medium | TBD | Open | 已拆出 `TowerDefinitionParser` 和 `TowerDefinitionValidator`，并保留 `TowerConfig` public API 兼容入口；后续继续移出 UI spec，并删除不再需要的 `splash_radius_cells`、`slow_*` 等 legacy fallback。 |
| TD-013 | 经济配置 | 若建塔费用只放在全局经济配置中，不同塔无法独立做建造成本平衡。 | Medium | TBD | Mitigated | `towers.json` 已增加 per-tower `build_cost`，放置、HUD、退款累计投入、schema 和 smoke 已跟随；`basic_tower_cost` 保留为兼容 fallback/default。 |
